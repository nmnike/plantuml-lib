...
Процедура ЗаполнитьИсходникБиблиотекиПоУмолчанию()   
	...
	ИсходныйКодПодключаемойБиблиотеки =
		...
		|!include v8/v8_InfoRg.puml
		|!include v8/v8_AccRg.puml
		|!include v8/v8_ChartOfCharacteristicTypes.puml
		|!include v8/v8_DefinedType.puml
		|!include v8/v8_Enum.puml
		|'<<<
		|";
...
Процедура СгенерироватьКодНаСервере()
	...
	Для Каждого ЭлементСписка Из ВыбранныеОбъектыМетаданных Цикл
		...
		ИначеЕсли СтрНайти(ЭлементСписка.Значение, "ПланВидовХарактеристик.") Тогда
			...
			СтрокаМетаданные = СформироватьСтрокуМетаданных("_ПланВидовХарактеристик", МетаданныеОбъекта); 
			ДополнитьОписаниемКоллекции(СтрокаМетаданные, МетаданныеОбъекта, СборкаВзаимосвязей,
				ВыбранныеОбъектыМетаданных);
...
Функция СгенерированноеИзображение(Данные, Параметры) 
	...
	Соединение = Новый HTTPСоединение(Параметры.СерверРендеринга, , , , , Параметры.ТаймаутСоединения, Параметры.ЗащищенноеСоединение);
	ТекстЗапроса = РесурсСервера + ФорматPlantUML;
	Если Отладка Тогда
		Сообщить(СтрШаблон("%1%2", Параметры.СерверРендеринга, ТекстЗапроса));
	КонецЕсли;
...
Функция ПараметрыПоУмолчанию()   
	...
	Результат = Новый Структура;
	Результат.Вставить("СерверРендеринга", "onyx-crs.onyx.local"); // www.plantuml.com
	Результат.Вставить("РесурсСервера", "/png/");	// /plantuml/png/
	Результат.Вставить("ТаймаутСоединения", 180);
	Результат.Вставить("ЗащищенноеСоединение", ?(HTTPS, Новый ЗащищенноеСоединениеOpenSSL, Неопределено));
	Возврат  Результат;
